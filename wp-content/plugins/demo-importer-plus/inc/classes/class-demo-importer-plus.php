...
if ( ! class_exists( 'Demo_Importer_Plus' ) ) {
	...
	class Demo_Importer_Plus {
		...
		public function admin_enqueue( $hook = '' ) {
			...
			wp_register_script( 'demo-importer-plus-admin-page', DEMO_IMPORTER_PLUS_URI . 'assets/admin/js/demo-importer-plus-admin.js', array( 'jquery', 'wp-util', 'updates', 'jquery-ui-autocomplete', 'demo-importer-plus-api' ), DEMO_IMPORTER_PLUS_VER, true );
		...
		public function get_local_vars() {
			...
			$data                 = apply_filters(
				...
				array(
					...
					'importFailedMessage'                => sprintf( __( '<p>Your website is facing a temporary issue in connecting the template server.</p><p>Read <a href="%s" target="_blank">article</a> to resolve the issue and continue importing template.</p>', 'demo-importer-plus' ), esc_url( 'https://rishidemos.com/' ) ),
					/* translators: %s is a documentation link. */
					'importFailedRequiredPluginsMessage' => sprintf( __( '<p>Your website is facing a temporary issue in connecting the template server.</p><p>Read <a href="%s" target="_blank">article</a> to resolve the issue and continue importing template.</p>', 'demo-importer-plus' ), esc_url( 'https://rishidemos.com/' ) ),
		...
		public function get_all_sites() {
			...
			if ( ! empty( $allowed_sites ) ) {
				...
				$sites_and_pages = wp_remote_get( DEMO_IMPORTER_PLUS_MAIN_DEMO_URI . '/wp-json/demoimporterplusapi/v1/dipa-demos/?per_page=100' );
			}
