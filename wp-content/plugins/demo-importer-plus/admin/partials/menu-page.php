...
<div id="demo-import-plus" class="themes wp-clearfix theme-browser">
	<h3 class="di-sites__search-title"><?php esc_html_e( 'Available Templates', 'demo-importer-plus' ); ?></h3>
	<div class="dip-sites__search-wrap">
	<?php foreach ( $page_builder_sites as $key => $site ) : ?>
		<div class="theme demo-import-plus-theme site-single has-pages demo-import-plus-previewing-site <?php echo 'free' !== $site['site_type'] ? 'premium-layout' : ''; ?>" data-site-id="<?php echo esc_attr( $key ); ?>"
			data-page-id="">
			<div class="inner">
				<span class="site-preview" data-title="<?php echo esc_attr( $site['site_title'] ) ?>">
					<div class="btn-wrap">
						<a target="_blank" href="<?php echo esc_url( $site['site_url'] ); ?>" class="button button-hero button-primary preview-button" target="_blank"><?php echo esc_html__( "Preview", "demo-importer-plus" ) ?></a>
					</div>
					<div class="theme-screenshot one loaded"
						data-featured-src="<?php echo esc_url( $site['site_featured_image'] ); ?>"
						style="background-image: url(&quot;<?php echo esc_url( $site['site_featured_image'] ); ?>&quot;);">
				...
				<div class="theme-id-container">
					<div class="theme-name">
						<span class="title">
							<div class="site-title"><?php echo esc_html( $site['site_title'] ); ?></div>
							<div class="pages-count"><?php printf( _nx( '%s template', '%s templates', count( $site['pages'] ), 'no. of templates', 'demo-importer-plus' ), number_format_i18n( count( $site['pages'] ) ) ); ?></div>
						</span>
					</div>
					<div class="favorite-action-wrap" data-favorite="" title="Make as Favorite">
	...
	<div class="single-site-wrap">
		<div class="single-site">
			<div class="single-site-pages-header">
				<h3 class="dip-site-title">{{{data['title']}}}</h3>
...
<script type="text/template" id="tmpl-demo-importr-plus-request-failed">
	...
	<p>
		...
		printf( __( 'Read an article <a href="%s" target="_blank">here</a> to resolve the issue.', 'demo-importer-plus' ), 'https://rishidemos.com/' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		?>
	</p>
</script>
...
<script type="text/template" id="tmpl-demo-importer-plus-page-builder-sites-search">
	...
	<# if ( sites_list_arr.length > 0 ) { #>
		<h3 class="di-sites__search-title"><?php esc_html_e( 'Site Templates', 'demo-importer-plus' ); ?></h3>
		<div class="dip-sites__search-wrap">
...
<# for ( site_id in data ) { #>
		<#
			...
			var favorite_class      = '';
			var favorite_title      = '<?php esc_html_e( 'Make as Favorite', 'demo-importer-plus' ); ?>';
			var featured_image_url = data[site_id]['site_featured_image'];
			var thumbnail_image_url = featured_image_url;

			var site_type = data[site_id]['site_type'] || '';
			var page_id = '';

			var title = data[site_id]['site_title'] || '';
